...
def run_muscle(input_fasta, output_fasta, muscle_cmd, log_file_path):
    ...
    log_message(log_file_path, "Running:\n"+" ".join(cmd))
    try:
...
def save_hotspot(hotspot_list, output_dir, structures, mode,
                 favicon_url="https://raw.githubusercontent.com/morth-lab/EnZight/main/favicon.ico"):
    ...
    legend = ("Each row is colored by its EnZight score" if mode == 1 else "Each row is colored by its EnZight score. The EnZight score is averaged for double mutations.")
...
def update_msa(msa_dicts, structures, cmd, threshold=6.0):
    ...
    cluster_score = []  # list of EnZight scores for each cluster
    ...
    # print(splited)
    # Iderate through clusters in order of EnZight score, merging nearest neighbors
